...
-- Sample attendance data insertion removed to prevent automatic record creation on deployment
...
-- NSSF Registration table
CREATE TABLE IF NOT EXISTS nssf_registration (
  id INT AUTO_INCREMENT PRIMARY KEY,
  registration_number VARCHAR(50) UNIQUE NOT NULL,
  employee_id INT NOT NULL,
  ...
  FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL,
  INDEX idx_registration_number (registration_number),
  INDEX idx_nssf_number (nssf_number),
  INDEX idx_employee_id (employee_id),
  INDEX idx_status (status)
);

-- Discipline Monitoring table
CREATE TABLE IF NOT EXISTS discipline_monitoring (
  id INT AUTO_INCREMENT PRIMARY KEY,
...
-- Office Resources Assignment table
...
CREATE TABLE IF NOT EXISTS talent_acquisition (
  ...
  FOREIGN KEY (approved_by) REFERENCES users(id) ON DELETE SET NULL,
  FOREIGN KEY (hiring_manager) REFERENCES users(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS promotions (
  id INT AUTO_INCREMENT PRIMARY KEY,
  promotion_number VARCHAR(50) UNIQUE NOT NULL,
  employee_id INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS risk_management (
  ...
  likelihood_after_mitigation ENUM('Very Low', 'Low', 'Medium', 'High', 'Very High'),
  impact_after_mitigation ENUM('Very Low', 'Low', 'Medium', 'High', 'Very High'),
  ...
  INDEX idx_risk_number (risk_number),
