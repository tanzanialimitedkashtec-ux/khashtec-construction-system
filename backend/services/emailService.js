...
/**
 * Build the branded HTML email template for payment notifications
 */
...
async function sendPaymentNotification(toEmail, details) {
    if (!RESEND_API_KEY) {
        console.warn('⚠️ Skipping payment email — RESEND_API_KEY not configured');
        return false;
    }
    ...
    try {
        const result = await sendViaResend(toEmail, subject, html);
        console.log(`✅ Payment email sent to ${toEmail} (ID: ${result.id})`);
        return true;
    } catch (error) {
...
module.exports = { sendInvoiceEmail, sendExpenseEmail, sendAssignmentNotification, sendPaymentNotification };
