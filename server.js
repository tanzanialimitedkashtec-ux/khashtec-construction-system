...
const suggestionsViewRoutes = require('./backend/routes/suggestionsView');
...
const accountantRoutes = require('./backend/routes/accountant');
...
// Force HTTPS redirection (Railway sets x-forwarded-proto)
...
async function createWorkerAssignmentsTable() {
    ...
    try {
        ...
        console.log('Creating worker_assignments table with IF NOT EXISTS...');
        ...
        // Verify table was created correctly and has required columns
        ...
        const hasRoleColumn = verification.some(col => col.Field === 'role_in_project');

        

        if (!hasRoleColumn) {

            console.log('role_in_project column missing, adding it...');

            await db.execute("ALTER TABLE worker_assignments ADD COLUMN role_in_project VARCHAR(255) NOT NULL DEFAULT 'Assigned'");

            console.log('role_in_project column added successfully');

        } else {

            console.log('worker_assignments table exists with correct schema');
